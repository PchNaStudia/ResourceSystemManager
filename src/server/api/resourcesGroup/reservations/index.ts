...
import {GetReservationsSchema, ReservationApprovalSchema, ReserveSchema} from "@common/ApiTypes";
...
reservationsRouter.get('/', async (req, res)=>{
  try {
    ...
    const canViewAll = !req.resourceAccess || req.resourceAccess.reserveLevel === 'APPROVE';
    const reservations = await db.select({reservation, resource}).from(reservation).innerJoin(resourceToReservation, eq(reservation.id, resourceToReservation.reservationId))
      .innerJoin(resource, eq(resourceToReservation.resourceId, resource.id))
      .where((canViewAll ? eq(reservation.groupId, req.resourceGroup!.id) : and(
...
reservationsRouter.put('/:id', async (req, res)=>{
  try{
    ...
    if(reserveLevel === 'CANCELED') {
      const [result] = await db.delete(reservation).where(and(
        eq(reservation.userId, req.user!.id),
    ...
    if(req.resourceAccess && req.resourceAccess.reserveLevel !== 'APPROVE') {
      res.status(401).json({error: "You do not have permission to approve reservations"})
      return;
    }
    const [result] = await db.update(reservation).set({
      status: reserveLevel,
...
reservationsRouter.post('/', async (req, res)=>{
  try{
    if(req.resourceAccess && req.resourceAccess.reserveLevel === 'NONE') {
      res.status(401).json({error: "You do not have permission to reserve resources"})
      return;
    }
    const reserveData = ReserveSchema.parse(req.body)
    await db.transaction(async (tx) => {
      await tx.execute(sql`SELECT * FROM ${resource} WHERE ${resource.id} IN (${reserveData.resources}) AND ${resource.groupId} = ${req.resourceGroup!.id} FOR UPDATE`)
      ...
      if(!reserveData.resources.every((v)=>resources.some((r)=>r.id === v))){
        res.status(400).json({error: "Some of the resources do not exist in resource group"})
        tx.rollback();
        return;
      }
      const conflicts = await tx.select({status:reservation.status, id: reservation.id}).from(reservation)
        .innerJoin(resourceToReservation, eq(resourceToReservation.reservationId, reservation.id))
        .where(and(
      ...
      if (conflicts.some((v) => v.status === 'CONFIRMED')) {
        res.status(409).json({error: "Some of the resources are already reserved", hard: true})
        tx.rollback()
        return;
      }
        ...
        if(reserveData.force){
          if(req.resourceAccess && req.resourceAccess.reserveLevel !== 'APPROVE') {
            ...
            await db.update(reservation).set({
              status: 'REJECTED',
              approvedBy: req.user!.id,
            }).where(inArray(reservation.id, conflicts.map((v)=>v.id)))
          }
        } else {
          res.status(409).json({error: "Some of the resources are already requested", hard: false})
          tx.rollback()
          return;
        }
      }
      const reservationId = (await tx.insert(reservation).values(values).$returningId())[0]
      tx.insert(resourceToReservation).values(reserveData.resources.map((v)=>({
        resourceId: v,
