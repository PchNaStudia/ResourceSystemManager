...
const typeExists = async (typeId: number, groupId: number) =>
  (
    await db
      .select()
      .from(resourceType)
      .where(
        and(eq(resourceType.groupId, groupId), eq(resourceType.id, typeId)),
      )
      .limit(1)
  ).length > 0;
...
resourceRouter.get("/", async (req, res) => {
  ...
  try {
    const resources = await db
      .select()
      .from(resource)
      .where(eq(resource.groupId, req.resourceGroup!.id));
    try {
  ...
  while (queue.length > 0) {
    queue = (
      await db
        ...
        .from(resourceType)
        .where(inArray(resourceType.parentId, queue))
    ).map((x) => x.id);
    typeId = [...typeId, ...queue];
  }
  return typeId;
...
resourceRouter.post("/", async (req, res) => {
  ...
  if (req.resourceAccess && !req.resourceAccess.create) {
    res
      .status(401)
      .json({ error: "You do not have permission to create resources" });
    return;
  }
  try {
...
resourceRouter.put("/:id", async (req, res) => {
  ...
  try {
    ...
    if (
      resourceData.typeId &&
      !(await typeExists(resourceData.typeId, req.resourceGroup!.id))
    ) {
      res.status(404).json({ error: "Resource type not found" });
      return;
    }
