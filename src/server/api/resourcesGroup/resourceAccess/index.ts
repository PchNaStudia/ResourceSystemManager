...
resourceAccessRouter.use((req, res, next) => {
  if (req.resourceAccess && !req.resourceAccess.manageAccess) {
    res
      .status(401)
      .json({ error: "You do not have permission to manage access" });
    return;
  }
  next();
});
...
resourceAccessRouter.post("/:userId", async (req, res) => {
  try {
    ...
    res
      .status(409)
      .json({ error: "User already has access to this resource group" });
    return;
  }
});
...
resourceAccessRouter.put("/:userId", async (req, res) => {
  try {
    if (req.resourceAccess && !req.resourceAccess.manageAccess) {
      res
        .status(401)
        .json({ error: "You can not edit permissions of other users" });
      return;
    }
    const userId = z.string().parse(req.params.userId);
    const userAccessData = CreateResourceGroupAccessSchema.parse(req.body);
    ...
    if (result.affectedRows === 0) {
      res
        .status(404)
        .json({ error: "User does not have access to this resource group" });
      return;
    }
    res.status(200).send();
  } catch {
...
resourceAccessRouter.delete("/:userId", async (req, res) => {
  try {
    if (req.resourceAccess && !req.resourceAccess.manageAccess) {
      res
        .status(401)
        .json({ error: "You can not remove permissions from other users" });
      return;
    }
    const userId = z.string().parse(req.params.userId);
    const [result] = await db
      .delete(resourcesAccess)
      .where(
        and(
          eq(resourcesAccess.userId, userId),
          eq(resourcesAccess.groupId, req.resourceGroup!.id),
          req.resourceAccess
            ? eq(resourcesAccess.manageAccess, false)
            : undefined,
        ),
      );
    if (result.affectedRows === 0) {
