...
resourcesGroupRouter.get("/", async (req, res) => {
  const access = await db
    .select({
      resourceGroups: resourcesGroups,
    ...
    .leftJoin(
      resourcesAccess,
      and(
        eq(resourcesGroups.id, resourcesAccess.groupId),
        eq(resourcesAccess.userId, req.user!.id),
      ),
    )
    .where(
      or(
        eq(resourcesAccess.userId, req.user!.id),
        eq(resourcesGroups.ownerId, req.user!.id),
      ),
    );
...
const resourcesGroupMiddleware = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
resourcesGroupRouter.use(
  "/:id/resource",
  resourcesGroupMiddleware,
  resourceRouter,
);
